...
parser.add_argument("--note", type=str, default='Unidirectional')
...
parser.add_argument("--rin_weight", type=float, default=0.75)
...
# Model dan Training Parameter Configurations
# Hidden Units
...
def train(data, dir='./', task='I'):
    ...
    for i, xdata in enumerate(data):
        ...
        x_bar = (m * x) + ((1 - m) * x_hat.view(B, T, V))
        unc = (m * torch.zeros(B, T, V).to(device)) + ((1 - m) * torch.exp(0.5 * dec_logvar).view(B, T, V))

        # RIN
        x_imp, y_out, y_score, xreg_loss, xus, xrs = rin(x, x_hat.view(B, T, V), unc, m, d, y)

        # Loss
        if task == 'C':
            loss_rin = criterion_rin(y_out, y.unsqueeze(1))
        else:
            loss_rin = 0
        ...
        loss_imp = (args.rin_weight * xreg_loss) + (args.vae_weight * loss_vae)
...
# Define Loaders
train_loader = sample_loader('train', train_data, train_label, args.batchsize, args.removal_percent, args.task)
valid_loader = sample_loader('valid', valid_data, valid_label, args.batchsize, args.removal_percent, args.task)
test_loader = sample_loader('test', test_data, test_label, args.batchsize, args.removal_percent, args.task)
...
rin = RIN(args).to(device)

total_params = sum(p.numel() for p in vae.parameters() if p.requires_grad)
total_params += sum(p.numel() for p in rin.parameters() if p.requires_grad)
writelog(f, 'Total params is {}'.format(total_params))

# Define Optimizer
optimizer = optim.Adam(list(vae.parameters()) + list(rin.parameters()), lr=args.lr, weight_decay=args.lambda2)
...
for epoch in range(args.epoch):
    ...
    if args.task == 'I':
        ...
        if mae < valid['mae']:
            ...
            torch.save(rin, '%s/model/rnn_%d_%d.pt' % (dir, args.fold, epoch))
